...
class Hand:
    ...
    def post_blinds(self):
        if self.headsup:
            self.pot += self.positions['BU'].bet(Hand.small_blind_amt) + self.positions['BB'].bet(Hand.big_blind_amt)
        elif not self.headsup:
        ...
        while p < m:
            ...
            if player_to_act.allin or player_to_act.folded:
                ...
                if player_action == 'CHECK':
                    print('p:{} start:{}  m:{}  i:{}  {} bet:{}   handCurrentBet:{}   playerCurrentBet:{}    Pot:{}  allin?:{}   Pos:{}   {}'.format(p,start,m,self.preflop_positions.index(player_to_act),player_action,bet, self.current_bet, player_to_act.player_current_bet, self.pot, player_to_act.allin,player_to_act.position, player_to_act))
                    p += 1
